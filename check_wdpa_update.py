# System Env: ArcGIS 10.3+ and ArcGIS Pro
# Author: Yichuan Shi
# Date: Feb 7, 2017; Aug 30 2019 rev
# Description: Produce a table of update, change and delete for any two given WDPA releases. Due to its exhaustive nature of implmentation, it is advised to run in smaller chunks, for example, by country, i.e. the same way updates are undertaken. 
# Usage: Run as a standalone script `main(old_data_path, new_data_path, out_file)`
...
def geom_description(geom):
    ...
    if geom.type == 'polygon':
        try:
            ...
            return 'Error in retrieving polygon information'
    
    # point
    else:
        try:
            return 'point_count:{1}; part_count:{2}; centroid(xy):{3:.6f}-{4:.6f}'.format(
            geom.pointCount,
            geom.partCount,
            geom.centroid.X,
            geom.centroid.Y)
            ...
            return 'Error in retrieving point information'
...
def main(old_data_path, new_data_path, out_file):
    ...
    with arcpy.da.SearchCursor(old_data, FIELDS_LIST) as cur:
        ...
        for row_count, o_row in enumerate(cur):
            ...
            # tracking progress
            if row_count % 100 == 0:
                print('Scanned rows:' + str(row_count))
    ...
    # 3. write to output
    with codecs.open(out_file, mode='wb', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerows(q)
